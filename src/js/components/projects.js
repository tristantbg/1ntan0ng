...
const Projects = {
    ...
    init: () => {
        ...
        Projects.fullpage = new fullpage('#elevator', {
            ...
            afterRender: function() {
                ...
                document.getElementById('projects').querySelectorAll('img').forEach(p => {
                    // p.classList.add('lazyload')
                })
    ...
    flickity: (element, options) => {
        ...
        flickityInstance.on('select', function() {
            if (this.selectedElement) {
                if (flickityInstance.caption) flickityInstance.caption.innerHTML = this.selectedElement.getAttribute('data-caption');
                if (flickityInstance.slideNumber) flickityInstance.slideNumber.innerHTML = `${this.selectedIndex + 1}/${this.cells.length}`;
                if (Projects.globalSlideNumber) Projects.globalSlideNumber.innerHTML = `${this.selectedIndex + 1}/${this.cells.length}`;
            }
        });
